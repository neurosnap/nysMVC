...
Class jsCore
	...
	Public Property Set Pair(p, v)
		...
		If TypeName(v) <> "jsCore" Then
			Err.Raise &hD, "class: class", "Object Type: '" & TypeName(v) & "'"
		End If
		Set Collection(p) = v
	End Property
...
Function QueryToJSON(dbc, sql)
	     'Response.Write "/*" & vbCrLf & sql & vbCrLf & "*/" & vbCrLf & vbCrLf & vbCrLf

        Dim rs, jsa, rec_count
...
Function QueryToJSONNoComment(dbc, sql)
        Dim rs, jsa, rec_count
        Set rs = dbc.Execute(sql)
        Set jsa = jsArray()
        While Not (rs.EOF Or rs.BOF)
        ...
        rs.MoveNext
        Wend
        Set QueryToJSONNoComment = jsa
End Function


   'SS - 8/19/2008 4:42:03 PM
   ' takes a jsArray object, and outputs a string that is a csv
   ' make sure you set the content type, using:  Response.ContentType = "application/vnd.ms-excel"
   Function jsObjToCSV(js)
      Dim res
      if js.Count > 0 Then
         For Each x in js(0).collection
            res = res & x & vbTab
         Next
      end if
      res = res & vbCrLf
      For i = 0 to js.Count - 1
         For Each x in js(i).collection
            res = res & js(i)(x) & vbTab
         Next
         res = res & vbCrLf
      Next
