...
var nysReport = (function($) {
	...
	function init(that) {
		...
		$(that).find(that.settings.views.filters).html("");
		$(that).find(that.settings.views.chart).html("");
		$(that).find(that.settings.views.grid).html("");
		...
		if (that.settings.hasOwnProperty("reportContent")) {
			...
			$.ajax({
				...
				"beforeSend": function() {
					...
					$(that).find(that.settings.views.grid + " table").html(that.loading_dialog);
				},
				"success": function(ret) {

					if (ret.length > 0 && that.settings.reportContent.hasOwnProperty("mData")) {
					...
					if (that.settings.hasOwnProperty("chartContent")) {
						...
						if (that.settings.chartContent.hasOwnProperty("mData")) {
							chartView(that, that.settings.chartContent.mData(that.data.report));
						} else {
		...
		for (var i = 0; i < that.settings.drilldownContent.length; i++) {
			...
			(function(i) {
				...
				$.ajax({
					...
					"data": data,
					"beforeSend": function() {
						$("td.details.row" + row.rowIndex).html(that.loading_dialog);
					},
					"success": function(ret) {

						var el_row = $("td.details.row" + row.rowIndex);

						var ajax = that.settings.drilldownContent[i];

						if (ajax.hasOwnProperty("success")) {

							ajax.success(data, ret, el_row);
							...
							if (ret !== null && ret.length > 0) {
								...
								if (ajax.hasOwnProperty("options")) {
									...
									if (ajax.options.hasOwnProperty("manual")) {

										ajax.options.manual(data, ret, el_row);
										...
										var content = '<div class="row-fluid">' +
														'<div class="span6" style="float: left !important;">' +
															'<div class="box chart">' +
																...
																'<div class="content"><table class="' + ajax.id + ' ' + ((ajax.options.tbl_class) ? ajax.options.tbl_class : '') + '"></table></div>' +
															'</div>' +
														'</div>' +
								...
								el_row.append(content);

								var fin = jsonToDataTables(that, ret, false, false, ajax);
								...
								if (that.settings.reportContent.hasOwnProperty("options")) {
									...
									if (that.settings.reportContent.options.hasOwnProperty("tabletools")) {
										...
										dt_obj.oTableTools = {
											...
											"aButtons": ["copy", { "sExtends": "csv", "sTitle": "export" }, {"sExtends": "xls", "sTitle": "export" }, "pdf", "print"]
										};
								...
								ajax.dt_obj = el_row.find("." + ajax.id).dataTable(dt_obj);
	...
	function listen(that) {
			...
			var i = $.inArray(row, that.anOpen);
				...
				if (i === -1) {
					...
					$(this).html('<a href="#" onclick="return false;">' + 
											((that.settings.reportContent.options.hasOwnProperty("drilldown_title")) ? that.settings.reportContent.options.drilldown_title : 'Expand') + 
											'</a>');
		...
		$(that).on("click", ".launch_view", function(e) {

			e.preventDefault();

			if (that.settings.hasOwnProperty("reportContent")
					&& that.settings.reportContent.hasOwnProperty("options")
					&& that.settings.reportContent.options.hasOwnProperty("launchContent")) {

				that.settings.reportContent.options.launchContent.onLaunch($(this).attr("launch_ID"), getFilters(that));

			} else {
				error(that, "missing onLaunch(launch_ID, filters) function for reportContent.options.launchContent");
			}

		});

		if (that.settings.hasOwnProperty("filters")
				&& that.settings.filters.hasOwnProperty("fields")
				&& $.isArray(that.settings.filters.fields)) {

			//set event handler for checkbox for datepicker
			for (var i = 0; i < that.settings.filters.fields.length; i++) {

				field = that.settings.filters.fields[i];

					if (field.input == "datepicker"
						&& field.hasOwnProperty("options") 
						&& field.options.hasOwnProperty("default") 
						&& !field.options['default']) {

							(function(i) {

								$(that).on("click", "#chk_" + field.id, function(e) {

									field = that.settings.filters.fields[i];

									if ($("#" + field.id).is(":disabled"))
	...
	function filterView(that) {
		...
		if (that.settings.hasOwnProperty("filters") 
			...
			for (var i = 0; i < that.settings.filters.fields.length; i++) {
				...
				if (field.input == 'text') {
					...
					if (field.hasOwnProperty("options") 
						...
						content += '<input type="checkbox" name="chk_' + field.id + '" id="chk_' + field.id + '" class="field_checks" style="margin-right: 5px; margin-bottom: 15px;">';

						content += '<input type="text" name="' + field.id + '" id="' + field.id + '" value="' + 
											((field.hasOwnProperty("options") && field.options['default']) ? field.options['default'] : getDate()) +
											'" style="width: 80% !important;" disabled>';
				...
				//select dropdown input
					...
					if (field.hasOwnProperty("selectOptions") 
						...
						&& $.isArray(field.selectOptions.data)) {
					...
					//special group_by select
					} else if (field.id == 'group_by' && field.selectOptions.hasOwnProperty("group") && $.isArray(field.selectOptions.group)) {
						...
						for (var j = 0; j < field.selectOptions.group.length; j++)
							content = content + '<option value="' + field.selectOptions.group[j].value + '">' + field.selectOptions.group[j].text + '</option>';
			...
			var error_msg = "settings.filterView missing, cannot load filters";

			if (that.settings.hasOwnProperty("filters") 
					&& that.settings.filters.hasOwnProperty("error")) {
				that.settings.filters.error(that.find(that.settings.views.filters), error_msg);
			} else {
				error(that, error_msg);
			}
	...
	function gridView(that, data) {
		...
		if (data.length > 0) {
			...
			else if (that.settings.reportContent.options 
				&& that.settings.reportContent.hasOwnProperty("launchContent"))
		...
		if (data != null && data.length > 0 && typeof data != 'undefined') {
			...
			for (var j = 0; j < view.cols.length; j++) {
				...
				if (col.type == "group_by" && !drilldown_chart_view) {
					...
					for (var f = 0; f < that.settings.filters.fields.length; f++) {

						var filter = that.settings.filters.fields[f];

						if (filter.hasOwnProperty("selectOptions") && filter.selectOptions.hasOwnProperty("group")) {
							...
							if (col.hasOwnProperty("axis")) {
								...
								if (group_vals != null) {
									...
									for (var z = 0; z < filter.selectOptions.group.length; z++) {

										//for (var y = 0; y < group_vals.length; y++) {

											if (filter.selectOptions.group[z].value == group_vals) {

												//TODO: MAKE IT ACCEPT MORE THAN 1 COLUMN!
												switch (col.axis) {

													case "x":
														x = x + filter.selectOptions.group[z].text + ', ';
														break;

													case "y":
														y = y + filter.selectOptions.group[z].text + ', ';
						...
						if (filter_got) {
							...
							for (var f = 0; f < that.settings.filters.fields.length; f++) {
								...
								if (filter.hasOwnProperty("selectOptions") && filter.selectOptions.hasOwnProperty("group")) {
									...
									for (var a = 0; a < filter.selectOptions.group.length; a++) {

										//for (var z = 0; z < filter_got.length; z++) {

											if (filter.selectOptions.group[a].value == filter_got) {
												...
												if (filter.selectOptions.group[a].hasOwnProperty("as")) {

													//var rec = String(filter.group[y].as);
													group_values.push(filter.selectOptions.group[a].as);

												} else {

													var rec = String(filter_got);
			...
			for (var i = 0; i < that.settings.filters.fields.length; i++) {
				...
				if (clear) {
					...
					if ($('#' + field.id).hasClass("hasDatepicker"))
						...
						if (field.input == "select2" && typeof $.fn.select2 === "function")
					...
					if ($("#chk_" + field.id).length > 0 && $("#chk_" + field.id).is(":checked")) {
						$("#chk_" + field.id).click();
					}

				} else {

					if ($('#' + field.id).val() != "" && $('#' + field.id).val() != null && !$("#" + field.id).is(":disabled")) {
	...
	function jsonToDataTables(that, data, expand, launch, drilldown_obj) {
		...
		if (typeof drilldown_obj === 'undefined')
			drilldown_obj = false;
		...
		for (var i = 0; i < data.length; i++) {
				...
				for (var prop in obj) {
					...
					if (that.settings.reportContent.hasOwnProperty("options") 
						&& that.settings.reportContent.options.hasOwnProperty("col_override")) {
							...
							ret.columns.push($.extend(true, options, that.settings.reportContent.options.col_override({ "key": prop, "value": obj[prop] })));
			...
			if (expand) {
				...
				row_agg = '<a href="#" onclick="return false;" mod_ID="' + data[i].ID + '">' + 
									((that.settings.reportContent.hasOwnProperty("options") 
											&& that.settings.reportContent.options.hasOwnProperty("drilldown_title")) ? 
												that.settings.reportContent.options.drilldown_title : 'Expand') + 
									'</a>,';
				...
				if (that.settings.reportContent.hasOwnProperty("options") 
						&& that.settings.reportContent.options.hasOwnProperty("launchContent"))
					lid = obj[that.settings.reportContent.options.launchContent.key];
				else
					lid = obj.ID;

				row_agg = '<a href="#" launch_ID="' + lid + '" class="launch_view">Launch</a>,';
			} else {
				row_agg = '';
			}
