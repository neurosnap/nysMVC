...
/* =======================================================================
 * nysReports.js v1.0.0
 * https://github.com/nysus/nysReports
 * Dependencies:  jQuery, 
 *				  DataTables
...
var nysReport = (function($) {
				...
				$.ajax({
					...
					"success": function(ret) {
						...
						if (ajax.hasOwnProperty("success")) {
							...
							ajax.success(ret, $("td.details.row" + row.rowIndex));
										...
										var content = '<div class="row-fluid">' +
														'<div class="span6" style="float: left !important;">' +
															'<div class="box chart">' +
																...
																'<div class="content"><table id="' + ajax.id + '" class="display dtbl"></table></div>' +
															'</div>' +
														'</div>' +
														'<div class="span6" style="float: right !important;">' + 
															'<div class="box chart">' +
																'<div class="title"><h4>' + ajax.options.loadChart.title + '</h4></div>' +
																'<div class="content" id="' + ajax.options.loadChart.id + '" style="height: 250px; width: 400px;"></div>' +
															'</div>' +
														'</div>'
													  '</div>';
 	...
 	function listen(that) {
		...
		$(that).on('click', that.settings.views.grid + ' td.expanded_view', function(e) {
		    ...
		    if (i === -1) {
			    ...
			    $(this).html('<a href="#" onclick="return false;">' + ((that.settings.drilldownContent.hasOwnProperty("title")) ? that.settings.drilldownContent.title : 'Expand') + '</a>');
			    oT.fnClose(row);
 	...
 	function filterView(that) {
 		...
 		if (that.settings.hasOwnProperty("filters")) {
			...
			var col_count = that.settings.filters.cols_per_row;
			var count = 0;
			...
			for (var i = 0; i < that.settings.filters.fields.length; i++) {
				...
				if (field.input == 'text') {
					...
					content = content + '<label class="form-label span' + span.label + '" for="' + field.id + '">' + field.title +
										': </label>' +
										'<div class="span' + span.input + '">' +
											'<input type="text" name="' + field.id + '" id="' + field.id + '" value="' + getDate() +
 	...
 	function gridView(that, data) {
		...
		if (data.length > 0) {
			...
			if (that.dtbl_obj != null) {
				...
				if ($(that).find(that.settings.views.grid).length > 0) {
					...
					parent.html('<table class="display dtbl"></table>');
		...
		//No data found for main query
			...
			if ($(that).find(that.settings.views.grid).length > 0) {
				...
				$(that).find(that.settings.views.grid).find("table").html('<tr><th></th></tr><tr><td>No data found with the current filters.</td></tr>');
 	...
 	function jsonToDataTables(that, data, expand, launch) {
		...
		for (var i = 0; i < data.length; i++) {
			...
			if (expand)
				row_agg = '<a href="#" onclick="return false;" mod_ID="' + data[i].ID + '">' + ((that.settings.drilldownContent.hasOwnProperty("title")) ? that.settings.drilldownContent.title : 'Expand') + '</a>,';
			else if (launch)
